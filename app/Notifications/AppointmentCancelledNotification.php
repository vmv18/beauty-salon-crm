...
class AppointmentCancelledNotification extends Notification
{
    ...
    {
        $reasonText = $this->reason ? " Причина: {$this->reason}" : '';
        ...
        if ($notifiable->hasRole('client')) {
            $message = "Ваш запис на {$this->appointment->service->name} скасовано.{$reasonText}";
        } else {
            $message = "Запис від {$this->appointment->client->user->name} скасовано.{$reasonText}";
        }
