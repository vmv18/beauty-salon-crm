...
@section('title', 'Редагувати майстра - Beauty Salon CRM')
...
@section('content')
    <div class="max-w-3xl mx-auto bg-white p-8 rounded-xl shadow-md">
        ...
        <h1 class="text-3xl font-bold text-purple-600 mb-8">✏️ Редагувати майстра: {{ $employee->user->name ?? 'N/A' }}</h1>
        ...
        <form method="POST" action="{{ route('employees.update', $employee) }}" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="specialization" name="specialization" value="{{ old('specialization', $employee->specialization) }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white focus:outline-none focus:border-purple-600 focus:ring-2 focus:ring-purple-200">
            ...
            <div class="mb-6">
                ...
                <textarea id="bio" name="bio" class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white resize-y min-h-[100px] focus:outline-none focus:border-purple-600 focus:ring-2 focus:ring-purple-200">{{ old('bio', $employee->bio) }}</textarea>
            ...
            <div class="mb-6">
                ...
                <div class="mb-2"><img src="{{ \App\Helpers\ImageHelper::getEmployeePhoto($employee) }}" alt="{{ $employee->user->name }}" class="max-w-[200px] mt-2 rounded-lg"></div>
            ...
            <div class="mb-6">
                ...
                <input type="number" id="rating" name="rating" value="{{ old('rating', $employee->rating) }}" step="0.01" min="0" max="5" class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white focus:outline-none focus:border-purple-600 focus:ring-2 focus:ring-purple-200">
            ...
            <div class="mb-6">
                ...
                <input type="date" id="hire_date" name="hire_date" value="{{ old('hire_date', $employee->hire_date?->format('Y-m-d')) }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white focus:outline-none focus:border-purple-600 focus:ring-2 focus:ring-purple-200">
            ...
            <div class="mb-6">
                ...
                <select id="status" name="status" class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white focus:outline-none focus:border-purple-600 focus:ring-2 focus:ring-purple-200">
                    <option value="active" {{ old('status', $employee->status) == 'active' ? 'selected' : '' }}>Активний</option>
                    <option value="inactive" {{ old('status', $employee->status) == 'inactive' ? 'selected' : '' }}>Неактивний</option>
                    <option value="on_leave" {{ old('status', $employee->status) == 'on_leave' ? 'selected' : '' }}>У відпустці</option>
                </select>
                ...
                <div class="flex flex-wrap gap-4 mt-2">
                    @foreach($services as $service)
                        <div class="flex items-center gap-2">
                            <input type="checkbox" id="service_{{ $service->id }}" name="services[]" value="{{ $service->id }}" {{ in_array($service->id, old('services', $employee->services->pluck('id')->toArray())) ? 'checked' : '' }} class="rounded">
            ...
            <div class="flex gap-4 mt-8">
                ...
                <a href="{{ route('employees.show', $employee) }}" class="px-6 py-3 bg-gray-600 text-white rounded-lg font-semibold hover:bg-gray-700 transition no-underline">Скасувати</a>
            </div>
        </form>
    </div>
@endsection
